...
void insert(const int &x, const int &st, queue<int> &Q, unordered_map<int,int> &d) {
  if (d[x] == 0) {
    d[x] = d[st]+1;
    Q.push(x);
  }
  ...
  while (!Q.empty()) {
    ...
    for (int i = 0; i < n; ++i) {
      for (int j = i+1; j < n; ++j) {
        ...
        // i -> j
        if (st[i] + st[j] <= cont[j]) {
          insert(st_50 + st[i]*(pow[j] - pow[i]), st_50, Q, d);
        }
        // j -> i
        if (st[i] + st[j] <= cont[i]) {
          insert(st_50 + st[j]*(pow[i] - pow[j]), st_50, Q, d);
        }
        // op2.
        // fill i
        if (st[j] >= cont[i]-st[i] && st[i] != cont[i]) {
          insert(st_50 - (pow[j] - pow[i])*(cont[i] - st[i]), st_50, Q, d);
        }
        // fill j
        if (st[i] >= cont[j]-st[j] && st[j] != cont[j]) {
          insert(st_50 + (pow[j] - pow[i])*(cont[j] - st[j]), st_50, Q, d);
        }
      }
      // op3.
      if (s - st[i] >= res_s) {
        insert(st_50 - st[i]*pow[i], st_50, Q, d);
      }
    }
  }
  return -1;
